...
export class MissionService {
  ...
  async updateMission(title: string, content: string, missionId: string, userId: string, groupId: string,) {

    const group = await this.groupRepository.findGroupById(groupId);
    if (group.leaderUserId !== userId) {
      throw new UnauthorizedException();
    }
    const compeleteCount = await this.compeleteReopsitory.countCompletes(missionId)
    if (compeleteCount !== 0) {
      throw new BadRequestException()
    }
    return await this.missionRepository.updateMission(title, content, missionId);
  }
  ...
  async deleteMission(userId: string, role: string, groupId: string) {
    ...
    const compeleteCount = await this.compeleteReopsitory.countCompletes(mission.id)
    if (role !== 'admin') {
